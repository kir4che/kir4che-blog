...
export const PokemonCard = ({ stage = 'final' }) => {
  ...
  const setCardVars = useCallback((vars) => {
    ...
    const x = parseFloat(vars['--pointer-x']);
    const y = parseFloat(vars['--pointer-y']);
    if (!Number.isNaN(x)) pointerStateRef.current.x = x;
    if (!Number.isNaN(y)) pointerStateRef.current.y = y;
  }, []);
