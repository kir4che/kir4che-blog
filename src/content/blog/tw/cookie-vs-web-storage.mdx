...
<Table
  data={{
    ...
    rows: [
      ...
      ['安全性', '易受 XSS 攻擊，不適合存儲敏感資訊。', '易受 XSS 攻擊，不適合存儲敏感資訊。'],
      [
        '適用場景',
...
Session 本身不直接依賴 Cookie，但通常會用 Cookie 存放 Session ID，以實現伺服器端的會話管理，如：用戶登入後，伺服器生成 Session，將 Session ID 回傳給瀏覽器存放於 Cookie
