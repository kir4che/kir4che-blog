...
Cookie 是伺服器發送到瀏覽器的一小段資料，會在後續的每次 HTTP 請求中自動附加到 header，用於與伺服器交互。簡單來說，它是一種**瀏覽器的儲存機制**，可幫助伺服器辨識使用者狀態 ex. 在登入網站後，伺服器可透過 Cookie 記住你的身份，讓後續請求保持登入狀態。
...
- **會話 Cookie（Session Cookie）**：被儲存在瀏覽器的資料僅在當前會話（使用者打開瀏覽器到關閉瀏覽器的這段時間）有效，關閉瀏覽器後即被刪除。
- **永久 Cookie（Persistent Cookie）**：可設定 `expires` 或 `max-age` 屬性，資料將保存至指定時間或至使用者手動刪除。

> 現代瀏覽器可能會保存會話（重啟瀏覽器會恢復標籤頁），此情況下 Cookie 可能會被保留。

### 缺點

- **容量小**：大小限制約為 4KB，無法儲存大量資料。
...
3. 瀏覽器將 Cookie 儲存起來，並按照設定的屬性（ex. expires、max-age）管理。
4. 瀏覽器再次向相同伺服器發出請求時，會自動將該 Cookie 附加到 header，供伺服器識別及使用。

### 適用場景

- 儲存伺服器需要的資料 ex. Session ID 或身份驗證 Token
- 需要跨瀏覽器分頁同步的資料
- 儲存使用者偏好 ex. 語言設定或主題模式

---

## WebStorage

WebStorage 是一種由瀏覽器提供 Client 端的儲存機制，包含 LocalStorage 和 SessionStorage，相較於 Cookie，具有以下優勢：

- **容量提升**：通常約 **5MB**，相較於 Cookie 的 **4KB** 空間更大。
- **降低網路負擔**：資料不隨 HTTP 請求自動傳送，減少不必要的流量。
- **易於使用**：提供簡易的 API（ex. setItem、getItem）便於操作資料
...
<Table
  data={{
    ...
    rows: [
      [
        '儲存期限',
        '永久保存，除非使用者手動或程式刪除，否則會一直存在於瀏覽器中。',
        '僅在當前會話有效，關閉瀏覽器後即刪除。',
      ],
      ['跨會話存取', '✅（同一網域下）', '❌'],
      [
        '存放位置',
        '僅存在於 Client 端，不隨 HTTP 請求傳送到伺服器，但可手動傳輸 ex. 透過 AJAX。',
        '僅存在於 Client 端，不隨 HTTP 請求傳送到伺服器，但可手動傳輸 ex. 透過 AJAX。',
      ],
      ['容量限制', '約 5MB（視瀏覽器而定）', '約 5MB（視瀏覽器而定）'],
      ['安全性', '易受 XSS 攻擊，不適合儲存敏感資訊。', '易受 XSS 攻擊，不適合儲存敏感資訊。'],
      [
        '適用場景',
        '長期使用或需跨分頁同步的非敏感資料，如使用者偏好、暫存應用資料等，且取代 Cookie 儲存購物車資訊。',
...
Session 是伺服器端的一種狀態管理技術，用於實現**使用者與伺服器之間的狀態保持**，特別是在 HTTP 這種無狀態協定中。

- **儲存位置**：Session 資料儲存在伺服器端，而 Client 端只需保存 Session ID（通常透過 Cookie 或 URL 傳遞）。
- **生命週期**
...
- 如果發現使用者異常登入 ex.多地同時登入，可以強制使舊的 Session 失效。

### Session 與 Cookie 的協作

Session 本身不直接依賴 Cookie，但通常會用 Cookie 存放 Session ID，以實現伺服器端的會話管理 ex. 使用者登入後，伺服器生成 Session，將 Session ID 回傳給瀏覽器存放於 Cookie 中。後續請求中，瀏覽器自動攜帶該 Cookie，伺服器根據 Session ID 確認使用者身份。
