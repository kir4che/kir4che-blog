...
postId: 'deploy-express-typescript-to-vercel'
description: >-
...
> Vercel’s **Serverless Function** only run when triggered by a request. That means all API endpoints must be routed through `api/index.ts`, which serves as the entry point for the Express app. Additionally, request forwarding must be configured in `vercel.json` to ensure proper routing.

Backend Folder Structure:

<Image
  src="/blog/deploy-express-typescript-to-vercel/server-folder-structure.webp"
...
<Image
  src="/blog/deploy-express-typescript-to-vercel/express-on-vercel-deploy-success.webp"
